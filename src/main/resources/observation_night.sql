...
( SELECT sub1.name AS name, sub1.phenomenon AS phen, sub1.obsdate AS date
	...
	( SELECT sub2.name, sub2.obsdate, max(cnt)
		FROM (
		SELECT name, phenomenon, CAST(("timestamp"+interval '3 hours') AS date) AS obsdate, count(*) AS cnt
...
( SELECT name AS name, round(avg(CAST(airtemperature AS numeric)),2) AS temp, CAST(("timestamp"+interval '3 hours') AS date) AS date
		FROM station, observation
		WHERE station.observationid=observation.observationid
			AND CAST(("timestamp"+interval '3 hours') AS time)  between CAST('00:00:00' AS time) AND CAST('08:00:00' AS time)
		GROUP BY name, date	
